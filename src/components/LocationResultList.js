...
const LocationResultList = ({ places, onSelectPlace, onSelectCurrent }) => (
  <Box width="100%" opacity={0.8} mt="1px">
    {places.length === 0
      && <LScript textAlign="center" m={3} fontSize={2} opacity={0.8} textKey="locationSearch:typeAbove" />}
    { places.map(place => (
    ...
    <TouchableOpacity onPress={onSelectCurrent}>
      <Box p={3} mb="1px" position="relative" flexDirection="row" alignItems="center">
        ...
        <LScript color="primary" fontSize={2} ml={2} textKey="locationSearch:currentLocation" />
      </Box>
    </TouchableOpacity>
  </Box>
);
